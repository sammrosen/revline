 ...
 * - Returns 404 if user already exists (no enumeration)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { password, confirmPassword, setupCode: providedCode, email } = body;
    ...
    const userEmail = email || `admin-${Date.now()}@placeholder.local`;
