...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Verify user has ADMIN or higher access to create workflows
    const access = await getWorkspaceAccess(userId, data.workspaceId);
    if (!access) {
      return ApiResponse.error('Workspace not found', 404, ErrorCodes.NOT_FOUND);
    }
    
    if (access.role !== WorkspaceRole.ADMIN && access.role !== WorkspaceRole.OWNER) {
      return ApiResponse.error('Insufficient permissions to create workflows', 403, ErrorCodes.UNAUTHORIZED);
    }
