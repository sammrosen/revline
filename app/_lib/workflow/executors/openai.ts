/**
 * OpenAI Action Executors
 *
 * Executors for OpenAI text generation operations.
 * Uses the OpenAIAdapter for API calls.
 *
 ...
 * - systemPrompt: Optional developer/system instructions for the AI.
...
const generateText: ActionExecutor = {
  async execute(
    ...
    const adapter = await OpenAIAdapter.forWorkspace(ctx.workspaceId);
    if (!adapter) {
    ...
    if (!validation.valid) {
      return {
        success: false,
        error: `OpenAI configuration error: ${validation.errors.join(', ')}`,
      };
    }
    ...
    await emitEvent({
      ...
      leadId: ctx.leadId,
      system: EventSystem.OPENAI,
      eventType: result.success ? 'openai_text_generated' : 'openai_generation_failed',
...
export const openaiExecutors: Record<string, ActionExecutor> = {
