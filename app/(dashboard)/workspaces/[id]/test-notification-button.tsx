...
function TestAlertDialog({ workspaceId, onClose }: TestAlertDialogProps) {
  ...
  useEffect(() => {
    fetch(`/api/v1/workspaces/${workspaceId}/test-alert`)
      .then((res) => res.json())
      .then((data) => {
  ...
  async function fireTestAlert(scenario: ScenarioKey) {
    ...
    try {
      const res = await fetch(`/api/v1/workspaces/${workspaceId}/test-alert`, {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        ...
        const statusRes = await fetch(`/api/v1/workspaces/${workspaceId}/test-alert`);
