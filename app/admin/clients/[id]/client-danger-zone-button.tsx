...
interface ClientDangerZoneButtonProps {
  clientId: string;
  clientName: string;
...
export function ClientDangerZoneButton({
  clientId,
  clientName,
  currentStatus,
  isDropdownItem = false,
}: ClientDangerZoneButtonProps) {
  ...
  const expectedPauseText = `pause ${clientSlug}`;
  const expectedDeleteText = `delete ${clientSlug}`;
    ...
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-semibold text-red-400">Danger Zone - {clientName}</h3>
              <button
                onClick={() => {
                  setShowModal(false);
                  setPauseConfirmText('');
