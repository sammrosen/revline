...
export interface MailerLiteInsightsProps {
  workspaceId: string;
}

export default function MailerLiteInsights({ workspaceId }: MailerLiteInsightsProps) {
  ...
  useEffect(() => {
    async function fetchInsights() {
      try {
        const response = await fetch(`/api/v1/admin/workspaces/${workspaceId}/mailerlite-insights`);
        if (!response.ok) {
